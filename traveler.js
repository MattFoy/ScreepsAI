...
class Traveler {
  ...
  findTravelPath(origin, destination, options = {}) {
    ...
    if (options.useFindRoute || (options.useFindRoute === undefined &&
      Game.map.getRoomLinearDistance(origin.pos.roomName, destination.roomName) > 2)) {
      allowedRooms = this.findAllowedRooms(origin.pos.roomName, destination.roomName, options);
    }
  ...
  travelTo(creep, destination, options = {}) {
    ...
    if (options.allowHostile === undefined) {
      options.allowHostile = true;
    }
    if (options.preferHighway === undefined) {
    ...
    if (!travelData.dest || travelData.dest.x !== destination.x || travelData.dest.y !== destination.y ||
      travelData.dest.roomName !== destination.roomName) {
      delete travelData.path;
    }
    // pathfinding
    if (!travelData.path) {
